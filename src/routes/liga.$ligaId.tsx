    ...
    head: ({ params }) => {
      ...
      return {
        meta: [
          ...
          { name: "description", content: `${l?.name}: aktuelle Spiele, Ergebnisse, Tabelle und Historie der letzten 10
  Jahre.` },
        ],
      };
    },
  ...
  function LigaPage() {
    ...
    return (
      <main className="mx-auto max-w-6xl px-4 pb-12">
        <Link to="/" className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-primary
  hover:underline">
        ...
        <section className="mt-5">
            ...
            <>
              ...
              {upcoming.length === 0 ? (
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {upcoming.slice(0, 12).map((m: Match) => <MatchCard key={m.id} match={m} teams={teams} />)}
                </div>
              )}
            </>
          )}
          ...
          {tab === "ergebnisse" && (
            <>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {results.slice(0, 12).map((m: Match) => <ResultCard key={m.id} match={m} teams={teams} />)}
              </div>
            </>
          )}
          ...
          {tab === "tabelle" && (
            <>
              <h2 className="mb-3 font-display text-xl font-bold text-foreground">Aktuelle Tabelle ·
  {seasonLabel(CURRENT_SEASON)}</h2>
